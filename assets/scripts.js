...
$(document).on('click', "[data-func='add-to-cart']", function () {
  ...
  UrbanCart.cartAdd(package_id, options)
    .then((res) => {
      toast({
        content:
...
$(document).on('click', "[data-func='update-options']", function () {
  ...
  UrbanCart.cartSetOptions(package_id, options)
    .then(() => {
      toast({
        title: 'Pronto!',
...
$('[data-cart-action]').on('click', function () {
  ...
  if (action === 'increase') {
    UrbanCart.cartAdd(package_id).then((res) => {
  ...
  if (action === 'decrease') {
    UrbanCart.cartRemove(package_id).then((res) => {
  ...
  if (action === 'remove') {
    UrbanCart.cartSetQuantity(package_id, 0).then((res) => {
      window.location.reload()
    })
  }
})
...
$("[data-cart-action='input']").on('blur', function () {
  ...
  UrbanCart.cartSetQuantity(package_id, quantity).then((res) => {
    if (quantity <= 0) {
      window.location.reload()
    } else {
      updateCartPrice(res.data)
...
$('.discord-button').on('click', function (e) {
  ...
  UrbanCart.requestDiscord().then((data) => {
